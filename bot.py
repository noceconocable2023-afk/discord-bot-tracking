...
# Listado de estados oficiales
ESTADOS_OPCIONES = [
    app_commands.Choice(name="1. PENDIENTE DE ACEPTACIÓN", value="PENDIENTE DE ACEPTACIÓN"),
    app_commands.Choice(name="2. PENDIENTE DE APROBACIÓN", value="PENDIENTE DE APROBACIÓN"),
    app_commands.Choice(name="3. PENDIENTE DE COMPRA", value="PENDIENTE DE COMPRA"),
    app_commands.Choice(name="4. PENDIENTE DE ENVÍO", value="PENDIENTE DE ENVÍO"),
    app_commands.Choice(name="5. PENDIENTE DE RECOJO", value="PENDIENTE DE RECOJO"),
    app_commands.Choice(name="6. ENTREGADO", value="ENTREGADO"),
...
class ConfirmarAccion(discord.ui.View):
    ...
    async def confirmar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.accion == "cerrar":
            ...
            await interaction.response.edit_message(content=f"🔒 Requerimiento **{self.codigo}** cerrado.", view=None)
...
async def on_ready():
    ...
    print(f"✅ Bot operativo con estados fijos: {bot.user}")
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(
        title="📦 Sistema de Requerimientos (Estados Fijos)",
        description="Ahora los estados se eligen de una lista para evitar errores.",
        color=discord.Color.green()
    )
    embed.add_field(name="🆕 /crear", value="Nuevo registro con estado inicial.", inline=False)
    embed.add_field(name="🔄 /actualizar", value="Cambiar a la siguiente etapa del proceso.", inline=False)
    embed.add_field(name="⏮️ /retroceder", value="Volver al estado anterior.", inline=False)
    embed.add_field(name="🔍 /ver", value="Detalles e historial completo.", inline=False)
    embed.add_field(name="📋 /lista", value="Ver registros por categoría.", inline=False)
    embed.add_field(name="🔒 /cerrar", value="Finalizar registro.", inline=False)
    embed.add_field(name="🗑️ /eliminar", value="Borrar registro.", inline=False)
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="crear", description="Registrar nuevo requerimiento", guild=GUILD)
@app_commands.choices(estado=ESTADOS_OPCIONES)
async def crear(interaction: discord.Interaction, codigo: str, estado: app_commands.Choice[str]):
    data = load_data()
    if codigo in data:
    ...
    data[codigo] = {
        ...
        "cerrado": True if estado.value == "CANCELADO" else False,
        "cerrado_el": ahora if estado.value == "CANCELADO" else None,
        "historial": [f"Creado: {estado.value} ({ahora})"]
    }
    save_data(data)
    await interaction.response.send_message(f"✅ Requerimiento **{codigo}** creado como `{estado.value}`.")

@bot.tree.command(name="actualizar", description="Cambiar estado del requerimiento", guild=GUILD)
@app_commands.choices(nuevo_estado=ESTADOS_OPCIONES)
async def actualizar(interaction: discord.Interaction, codigo: str, nuevo_estado: app_commands.Choice[str]):
    data = load_data()
    if codigo not in data:
        return await interaction.response.send_message("❌ El registro no existe.", ephemeral=True)
    
    if data[codigo].get("cerrado") and data[codigo]["estado"] != "CANCELADO":
        return await interaction.response.send_message("❌ No se puede actualizar un registro cerrado.", ephemeral=True)

    ahora = get_lima_time()
    data[codigo]["estado"] = nuevo_estado.value
    data[codigo]["actualizado_el"] = ahora
    data[codigo]["historial"].append(f"Actualizado: {nuevo_estado.value} ({ahora})")
    
    if nuevo_estado.value == "CANCELADO":
        data[codigo]["cerrado"] = True
        data[codigo]["cerrado_el"] = ahora

    save_data(data)
    await interaction.response.send_message(f"🔄 **{codigo}** actualizado a: `{nuevo_estado.value}`.")
...
async def ver(interaction: discord.Interaction, codigo: str):
    ...
    estado_status = "🟢 ABIERTO" if not info["cerrado"] else f"🔴 FINALIZADO/CANCELADO"

    msg = (
        f"📋 **DETALLES: {codigo}**\n"
        f"**Estado Actual:** `{info['estado']}`\n"
        f"**Status:** {estado_status}\n\n"
        f"**Línea de Tiempo:**\n{historial}"
    )
...
async def retroceder(interaction: discord.Interaction, codigo: str):
    data = load_data()
    if codigo not in data: return await interaction.response.send_message("❌ No existe.", ephemeral=True)
    if len(data[codigo]["historial"]) <= 1: return await interaction.response.send_message("⚠️ Sin cambios previos.", ephemeral=True)

    await interaction.response.send_message(
        f"⏮️ ¿Deseas eliminar la última actualización de **{codigo}**?", 
...
@app_commands.choices(tipo=[
    ...
    app_commands.Choice(name="Cerrados/Cancelados", value="cerrados"),
...
async def lista(interaction: discord.Interaction, tipo: app_commands.Choice[str]):
    ...
    if not res: return await interaction.response.send_message("📭 Vacío.")
...
async def cerrar(interaction: discord.Interaction, codigo: str):
    ...
    await interaction.response.send_message(f"⚠️ ¿Confirmas el cierre de **{codigo}**?", view=ConfirmarAccion(codigo, "cerrar"))
